...
export const getTracks = function getTracks(token) {
  return request({
    uri: `https://api.soundcloud.com/me/tracks?client_id=${process.env.SC_CLIENT_ID}&oauth_token=${token}`,
    json: true
  })
}

export const getTrack = function getTrack(id, token) {
  console.log(`https://api.soundcloud.com/me/tracks/${id}?client_id=${process.env.SC_CLIENT_ID}&oauth_token=${token}`)
  return request({
    uri: `https://api.soundcloud.com/me/tracks/${id}?client_id=${process.env.SC_CLIENT_ID}&oauth_token=${token}`,
    json: true
  })
}
