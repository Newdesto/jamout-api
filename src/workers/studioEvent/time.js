...
queue.process('studio-session:inquire--time', async ({ data: { type, userId } }, done) => {

  // Get the events from API.ai
  const { response, events } = await eventRequestAndProcess({ name: 'studio-session:inquire--time' }, { sessionId: userId });
  ...
  pubsub.publish(`assistant.${userId}`, {
    ...
    next: 'studio-session:inquire--complete'
  })
