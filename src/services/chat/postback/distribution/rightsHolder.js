...
const rightsHolderHandler = async function rightsHolderHandler({ user, channelId, values }) {
  ...
  await Release.update(values.releaseId, { rightsHolder: values.rightsHolder })
  ...
  const metadataResult = await eventRequest({
    name: 'distribution-confirm-and-pay',
    data: {
...
export default rightsHolderHandler
